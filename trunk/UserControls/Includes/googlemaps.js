...
var GoogleMap = function(element, geo, url, options) {
	...
	var g = this;
	g._StartWork();
	g._LoadInRadius("LoadPeopleInRadius", geo, distance, callback, 0);
    };
	...
	var g = this;
	g._StartWork();
	g._LoadInRadius("LoadFamiliesInRadius", geo, distance, callback, 0);
    };
	...
	var g = this;
	g._StartWork();
	g._LoadInRadius("LoadGroupsInRadius", geo, distance, callback, 0);
    };
	...
	var g = this;
	g._StartWork();
	g._LoadInArea("LoadPeopleInArea", areaid, callback, 0);
    };
	...
	var g = this;
	g._StartWork();
	g._LoadInArea("LoadFamiliesInArea", areaid, callback, 0);
    };
	...
	var g = this;
	g._StartWork();
	g._LoadInArea("LoadGroupsInArea", areaid, callback, 0);
    };
    ...
    // finished is a true/false indicating if any more data will be returned.
    //
    this.LoadPeopleInRadius = function(address, distance, callback)
    {
	if (typeof(callback) == 'undefined' || callback == null)
	    callback = this._AddPeopleToMap;
	...
	g._StartWork();
	$.ajax({
		url: this.serviceurl + "/GeocodeAddress",
		data: JSON.stringify({
		...
		success: function(data) {
		    g._LoadInRadius("LoadPeopleInRadius", data.d, distance, callback, 0);
		},
		error: function() {
    ...
    // finished is a true/false indicating if any more data will be returned.
    //
    this.LoadFamiliesInRadius = function(address, distance, callback)
    {
	if (typeof(callback) == 'undefined' || callback == null)
	    callback = this._AddFamiliesToMap;
	...
	$.ajax({
		...
		dataType: "json",
		success: function(data) {
		    g._LoadInRadius("LoadFamiliesInRadius", data.d, distance, callback, 0);
		},
		error: function() {
		    callback(true, new Array(), false, g);
		    g._StopWork();
    ...
    //
    // Internal method to handle loading by radius in the background.
    // The cmd is the type of loading to perform as this method can
    // deal with multiple types, such as individuals, families and
    // small groups.
    //
    this._LoadInRadius = function(cmd, geo, distance, callback, offset) {
        var g = this;
	$.ajax({
		...
		success: function(data) {
			...
			g._LoadInRadius(cmd, geo, distance, callback, offset + 100);
		    else
			g._StopWork();
		},
		error: function() {
    ...
    //
    // Internal method to handle loading by area in the background.
    // The cmd is the type of loading to perform as this method can
    // deal with multiple types, such as individuals, families and
    // small groups.
    //
    this._LoadInArea = function(cmd, areaid, callback, offset) {
        var g = this;
	$.ajax({
		url: this.serviceurl + "/" + cmd,
		data: JSON.stringify({
		...
		success: function(data) {
		    callback(false, data.d, (data.d.length < 100), g);
		    if (data.d.length == 100)
			g._LoadInArea(cmd, areaid, callback, offset + 100);
		    else
			g._StopWork();
		},
		error: function() {
